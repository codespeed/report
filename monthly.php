<?php
  ...
  require('fpdf/fpdf.php');
   ...
   $report_title = "Report";
    ...
    if(isset($_GET["y"]) && isset($_GET["y"])){
      $rows = $healthcards->find(array("m"=>(string)$_GET["m"], "y"=>(int)$_GET["y"]));
  ...
  $rows2 = iterator_to_array($rows);
    foreach ($rows2 as $key => $row) {
        $new_row = array($row['hc_lastname'].", ".$row['hid'],$row['hc_firstname'],$row['hc_position'],$row['hc_job_category'],$row['hc_business_employment']);
